...
// this adds a listener startButtonHandler to the startbutton eventlistener
...
//if this is called with no paramaters it will default to level
// otherwise it uses switch to determine wich level up to 4 otherwise it will return an error 
function setLevel(level = 1) {
  switch (level) {
    ...
    case 3: return 20;
    case 4: return 31;
    case 5: return "Please enter level 1, 2, 3, or 4";
    case 6: return "Please enter level 1, 2, 3, or 4";
  }
...
function startButtonHandler() {
  // Set max rounds the default level
  maxRoundCount = setLevel();
  roundCount++; // new round
  startButton.classList.add("hidden"); // this hide start button after clicked
  statusSpan.classList.remove("hidden"); // gets rid of statusspam when start button is clicked
  playComputerTurn(); //this calls the players turn
  return { startButton, statusSpan }; //this returns paramaters to statbutton listener
}
...
function padHandler(event) {
  ...
  if (!color) return; // if it cant find a color then it returns
  // this finds the pad for the color
  let pad = pads.find(function (element) {
...
function getRandomItem(collection) {
  ...
  const randomIndex = Math.floor(Math.random() * collection.length); //limit to number of pads
...
function activatePad(color) {
  // TODO: find a pad color and if it matches return
  let pad = pads.find(function (element) {
    return element.color === color
  });
  pad.selector.classList.add("activated");//pad selector gets activated
  pad.sound.play(); //play sound
  // this gives delay for 500 miliseconds and remove pad selector activated
  setTimeout(() => {
...
function activatePads(sequence) {
  // after 600 ms activate pad for each pad
  let index = 0 //sets index to 0
  // goes through each pad
  sequence.forEach((element) => {
    //sets a delay for each activated pad
    setTimeout(() => {
      activatePad(element)
    },
    //increaments pads after 600ms
      ++index * 600
    )
  });
}
...
function playComputerTurn() {
  padContainer.classList.add("unclickable"); //block any clicks on computer turn
  setText(statusSpan, "The computer's turn..."); // sets status text
  setText(heading, `Round ${roundCount} of ${maxRoundCount}`); //sets heading to roundcount
  computerSequence.push(getRandomItem(pads).color); //this gets the pad color pushes to computersequence
  activatePads(computerSequence); //it activates a pad for 500 ms then unactivates
  //this delays 600ms * roundcount + 1 second and then its the players turn
  setTimeout(() => {
    playHumanTurn()
  },
    roundCount * 600 + 1000
...
function playHumanTurn() {
  // makes pads clickable
  padContainer.classList.remove("unclickable");
  setText(statusSpan, `You have ${computerSequence.length - playerSequence.length} turns left`); // sets text to show how many turns left
}
...
function checkPress(color) {
  playerSequence.push(color); //pushes the pad the player clicked
  let index = playerSequence.length - 1 //this sets the index to the last pad the player clicked and subtracts 1
  let remainingPresses = computerSequence.length - playerSequence.length; // finding how much player sequence differs from computer sequence
  setText(statusSpan, `You have ${remainingPresses} turns left`); // sets text to how many turns are left
  //if the player choose wrong then shows message you lost and rests game
  if (computerSequence[index] !== playerSequence[index]) {
    resetGame("You lost");
  }
  // otherwise checks remaining presses
  else if (remainingPresses === 0) {
    checkRound(); // shows what round your on checks to see if your done if not then start player sequance
  }
}
...
function checkRound() {
  // checks what round the players on if at max show text you win
  if (playerSequence.length === maxRoundCount) {
    resetGame("You win");
  }
  // else increase round by 1 
  else {
    roundCount++;
    playerSequence.length = 0; // empty the player array 
    setText(statusSpan, "Nice! Keep going!"); // sets text to nice! keep going!
    //sets a delay for computers turn to 1 second
    setTimeout(() => { 
      playComputerTurn();
    },
      1000
...
function resetGame(text) {
  // empty computer sequence array
  computerSequence.length = 0;
  playerSequence.length = 0; // empty player sequence array
  roundCount = 0; // sets round to 0
  // Uncomment the code below:
  alert(text); // alert the player with text thats passed in
  setText(heading, "Simon Says"); // sets text to simon says
  startButton.classList.remove("hidden"); //removes hidden start button
  statusSpan.classList.add("hidden"); // hides status
  padContainer.classList.add("unclickable"); // stops pads from being clicked
}
